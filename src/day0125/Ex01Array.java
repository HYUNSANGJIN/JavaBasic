...
/*
 * 배열(Array)
 * ㄴ 지정된 크기만큼의 똑같은 데이터타입이 모여있는 하나의 자료형
 * 
 * 
 * 다음과 같이 선언을 하게 된다
 * ㄴ 데이터타입[] 배열이름 = new 데이터타입[크기]
 * 
 * 배열은 우리가 맨 처음에 지정한 크기만 다룰 수 있다.
 * 예를 들어서 우리가 선언할때
 * ㄴ 크기를 4라고 하면 
 * ㄴ 총 4개의 데이터타입을 묶어서 다루게 된다.
 * 
 * 또한, 우리가 변수를 일일이 이름을 붙여서 다루는것이 아니라
 * 배열의  "몇번째" 라는 식으로 다루게 된다.
 * 
 * 예를 들어 우리가 int배열 intArray의 2번째에 값을 할당할 때에는
 * ㄴ intArray[2] = 30; 이런식으로 값을 할당하거나
 * 
 * 값을 호출할때에는
 * ㄴ System.out.println(intArray[2]); 이렇게 호출 가능
 * 
 * 기본형 데이터타입의 배열은 모든 위치를 0으로 초기화한다.
 * 참조형 데이터타입의 배열은 모든 위치를 null로 초기화 한다.(ex : String)
 * null - 특수한 "상태"로서, 참조형 데이터타입이 메모리 주소값을 부여가 되었지만
 *         ㄴ 해당 메모리 주소값을 참조해서 들어가면 아무것도 없는 상태를 null이라고 하고
 *         ㄴ null인 상태에서는 새로운 값을 할당하는 것 외에는 아무것도 할 수 없다.
 * 
 * 배열의 존재하지 않는 위치를 접근할려고 한다면
 * ㄴ ArrayIndexxOutOfBoundsException, 즉 잘못된 위치 접근 에러가 발생.
 * 
 * 위치를 지정하는 "위치 번호"는 우리가 index라고 부른다.
 * ㄴ 배열의 인덱스는 0부터 (해당 배열 크기 -1)까지이다.
 * 
...
public class Ex01Array {
	public static void main(String[] args) {
		// int 타입의 배열 intArray
		int[] intArray = new int[5];   // 총 5개의 int변수를 묶어놓은것.
